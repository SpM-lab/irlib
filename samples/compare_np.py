...
for Lambda in [1000.0]:
    b_np20 = irlib.loadtxt("np20/basis_f-mp-Lambda"+str(Lambda)+".txt")
    b_np10 = irlib.loadtxt("np10/basis_f-mp-Lambda"+str(Lambda)+".txt")

    print("dim = ", b_np20.dim(), b_np10.dim())

    plt.figure(4)
    #nvec = numpy.arange(100)
    #nvec = numpy.array([0,10,10**2,400, 600, 800, 10**3, 2000, 4000, 6000, 10**4,10**5,10**6])
    nvec = numpy.array([10,100])
    Tnl_np20 = b_np20.compute_Tnl(nvec)
    Tnl_np10 = b_np10.compute_Tnl(nvec)
    for l in [b_np20.dim()-1]:
        plt.plot(nvec, numpy.abs(Tnl_np20[:,l]), marker='x', linestyle='-', color='r', label='l='+str(l))
        plt.plot(nvec, numpy.abs(Tnl_np10[:,l]), marker='', linestyle='--', color='b', label='l='+str(l))
        plt.plot(nvec, numpy.abs(Tnl_np10[:,l]-Tnl_np20[:,l]), marker='x', linestyle='-', color='r', label='abs error')
        plt.plot(nvec, numpy.abs(Tnl_np10[:,l]-Tnl_np20[:,l])/numpy.abs(Tnl_np20[:,l]), marker='o', linestyle='-', color='r', label='relative error')
        print(Tnl_np10[:,-1])
        print(Tnl_np20[:,-1])
